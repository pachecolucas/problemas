/**
 * Transtornos de Personalidade segundo o DSM-5
 * Organizados pelos três grupos (clusters) da abordagem categórica.
 *
 * Observação: os critérios abaixo são uma PARÁFRASE dos critérios diagnósticos
 * do DSM-5, reescritos para uso em código. Para finalidade clínica, consulte
 * sempre o texto oficial do DSM-5 / DSM-5-TR.
 *
 * Para a maioria dos transtornos o DSM-5 exige um número mínimo de critérios
 * presentes (indicado no campo `minimoCriterios`), além de início no começo da
 * vida adulta e padrão estável e abrangente que cause sofrimento ou prejuízo.
 *
 * (*) Transtornos excluídos do modelo alternativo do DSM-5 por falta de dados
 * para validação como entidades distintas — marcados com `modeloAlternativo: false`.
 */
